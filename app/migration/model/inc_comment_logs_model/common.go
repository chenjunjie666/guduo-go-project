package inc_comment_logs_model
...
func sync(res []*Table) {
	...
	arr := make([]*comment_count_trend_daily_model.Table, 0, 400)
	for _, v := range res {
		row := &comment_count_trend_daily_model.Table{
			ShowId:     v.ShowId,
		...
		arr = append(arr, row)
		if len(arr) >= 400 {
			comment_count_trend_daily_model.Model().Create(&arr)
			arr = make([]*comment_count_trend_daily_model.Table, 0, 400)
		}
	}
	if len(arr) > 0 {
