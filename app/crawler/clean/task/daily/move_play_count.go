...
func moviePlayCountHandle() {
	...
	sidsTmp := show_model.GetActiveShowsByType(show_model.ShowTypeMovie, show_model.ShowSubTypeMovieCinema)
	var sids = make([]uint64, len(sidsTmp))
	for k, v := range sidsTmp {
		sids[k] = v.ID
	}
	...
	rankFin[0] = make([]*play_count_rank_model.Table, 0, 500);
	for _, v := range res {
		...
		rankFin[0] = append(rankFin[0], &play_count_rank_model.Table{
			ShowId:      v.ShowId,
			Num:         v.Num,
		})
	}
