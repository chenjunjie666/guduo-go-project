...
import (
	...
	"guduo/app/internal/model_clean/actor_domi_weekly_model"
...
func guduoActorDomiHandle() {
	...
	save := make([]actor_domi_weekly_model.Table, len(tmp), len(tmp) * 2)
	k := 0
	for aid, row := range tmp {
		name := actor_model.GetActorName(aid)
		for type_, hot := range row {
			save[k] = actor_domi_weekly_model.Table{
				ActorId: aid,
	...
	hot1 = task.ActorTopK(hot1, 200)
	hot2 = task.ActorTopK(hot2, 200)
	hot3 = task.ActorTopK(hot3, 200)
	...
	actor_domi_weekly_model.SaveWeeklyHot(save, StartAt)
}