...
import (
	...
	"guduo/app/internal/model_clean/actor_hot_weekly_model"
...
func guduoActorHotHandle() {
	...
	mdl.Select("actor_id", "IF(custom_num != 0, custom_num, num) as num").
	...
	save := make([]actor_hot_weekly_model.Table, len(tmp))
	k := 0
	for aid, hotSum := range tmp {

		name := actor_model.GetActorName(aid)
		save[k] = actor_hot_weekly_model.Table{
			ActorId: aid,
	...
	actor_hot_rank_model.SaveCurRank(hot, 0, model_clean.CycleWeekly, JobAt)
	actor_hot_rank_model.SaveCurRank(hot2, 1, model_clean.CycleWeekly, JobAt)


	actor_hot_weekly_model.SaveWeeklyHot(save, StartAt)
}