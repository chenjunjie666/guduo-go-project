...
func qihooHandle() {
	urls := storage.Qihoo.GetIndicatorActorUrl()

	for _, row := range urls {
		wg.Add(1)
		ch.PushJob()
		go qihooIndicator(row.Url, row.ActorId)
	}
	wg.Done()
}

// 爬取360指数
func qihooIndicator(u string, actorId uint64) {
	defer ch.PopJob()
	defer wg.Done()
	...
	c.OnResponse(func(r *colly.Response) {
		...
		storage.Qihoo.StoreIndicatorActor(lastDayDataInt, JobAt, actorId)
	})
